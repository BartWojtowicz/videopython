...
from videopython.ai.video_analysis import _phash_dedup_frames
...
def test_dedup_collapses_identical_frames_to_one() -> None:
    ...
    kept = _phash_dedup_frames(frames, max_distance=4)
    assert len(kept) == 1
...
def test_dedup_floor_keeps_at_least_one_frame() -> None:
    ...
    kept = _phash_dedup_frames(frames, max_distance=64)
    assert len(kept) >= 1


def test_dedup_passthrough_for_single_frame_input() -> None:
    frames: list[np.ndarray | Image.Image] = [_solid(0)]
    assert _phash_dedup_frames(frames, max_distance=4) is not frames  # returns a fresh list
    assert len(_phash_dedup_frames(frames, max_distance=4)) == 1
