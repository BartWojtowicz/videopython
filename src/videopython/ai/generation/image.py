...
class TextToImage:
    ...
    SUPPORTED_BACKENDS: list[str] = ["local", "openai", "replicate"]
    ...
    def __init__(
        ...
        """
        if backend is None:
            backend = get_default_backend("text_to_image")  # type: ignore

        if backend not in self.SUPPORTED_BACKENDS:
            raise UnsupportedBackendError(backend, self.SUPPORTED_BACKENDS)

        self.backend: TextToImageBackend = backend  # type: ignore
        self.model = model
    ...
    async def _generate_replicate(self, prompt: str) -> Image.Image:
        ...
        model_name = self.model or get_replicate_model("text_to_image")
        ...
        image_url = await asyncio.to_thread(_run_replicate)
